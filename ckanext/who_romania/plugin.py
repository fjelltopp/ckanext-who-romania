...
class WHORomaniaPlugin(plugins.SingletonPlugin, DefaultPermissionLabels):
    ...
    def declare_config_options(self, declaration, key):
        ...
        declaration.declare(
            group.lambda_ckan_url, "https://wrc.fjelltopp.org"
        ).set_description("CKAN url for lambda functions to act upon")
        declaration.declare(group.lambda_family_medicine_function, "").set_description(
            "The family medicine lambda function name"
        )
        declaration.declare(group.lambda_family_medicine_template, "").set_description(
            "The family medicine data reporting template"
        )
        declaration.declare(group.lambda_invoke_users, "").set_description(
            "Users (other than sysadmins) with permission to invoke"
        )

    # IBlueprint
    def get_blueprint(self):
        return [who_romania_blueprints.lambda_blueprint]
